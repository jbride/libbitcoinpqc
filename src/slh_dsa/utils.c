...
void slh_dsa_init_random_source(const uint8_t *random_data,
...
void custom_slh_randombytes_impl(uint8_t *out, size_t outlen) {
...
#ifdef __EMSCRIPTEN__
    /* For WASM, use Emscripten's getRandomValues via EM_ASM */
    /* The getRandomValues function is provided by the module config in
     * JavaScript */
    EM_ASM_(
        {
          var buffer = HEAP8.subarray($0, $0 + $1);
          // Create a Uint8Array view for getRandomValues
          var arr = new Uint8Array(buffer.buffer, buffer.byteOffset,
                                   buffer.byteLength);

          // Try to get getRandomValues from Module config or global crypto
          // Note: crypto.getRandomValues must be called with correct 'this'
          // context
          if (typeof Module !== 'undefined' && Module.getRandomValues) {
            // Module.getRandomValues (if provided) should work as-is
            Module.getRandomValues(arr);
          } else if (typeof crypto !== 'undefined' && crypto.getRandomValues) {
            // crypto.getRandomValues must be called with crypto as 'this'
            crypto.getRandomValues.call(crypto, arr);
          } else {
            /* Fallback: fill with zeros if getRandomValues not available */
            console.error(
                '[custom_slh_randombytes_impl] ERROR: getRandomValues not available!');
            for (var i = 0; i < $1; i++) {
              buffer[i] = 0;
            }
          }
  ...
  if (outlen > remaining) {
    /* If we need more random bytes than available, we cycle through the
     * provided data */
    size_t position = 0;

    while (position < outlen) {
      size_t to_copy =
          (outlen - position < remaining) ? outlen - position : remaining;
      memcpy(out + position, g_random_data + g_random_data_offset, to_copy);

      position += to_copy;
      g_random_data_offset =
          (g_random_data_offset + to_copy) % g_random_data_size;
...
void slh_dsa_derandomize(uint8_t *seed, const uint8_t *m, size_t mlen,
                         const uint8_t *sk) {
  ...
  if (combined) {
    ...
    for (size_t i = 0; i < 10; i++) {
      for (size_t j = 0; j < 64; j++) {
        buffer[j] =
            buffer[(j + 1) % 64] ^ buffer[(j + 7) % 64] ^ buffer[(j + 13) % 64];
      }
    }
